    ...
    <div className="space-y-10">
        ...
        <div className="flex justify-between items-end mb-10">
          ...
          <div className="flex bg-[#F0EDE7]">
            <button 
              ...
              className={`px-8 py-4 text-xs font-black uppercase tracking-widest transition-all ${activeTab === 'pending' ? 'bg-brand-primary text-white' : 'text-gray-400 hover:text-brand-primary'}`}
            >
              Requests ({pendingRequests.length})
            </button>
            <button 
              onClick={() => setActiveTab('tracking')}
              className={`px-8 py-4 text-xs font-black uppercase tracking-widest transition-all ${activeTab === 'tracking' ? 'bg-brand-primary text-white' : 'text-gray-400 hover:text-brand-primary'}`}
            >
              ...
              <div key={biz.id} className="bg-[#F9F7F2] p-8 border border-[#E5E1D8] flex flex-col group hover:border-brand-primary transition-all">
                ...
                <div className="flex gap-4 mt-auto">
                  <button 
                    ...
                    className="flex-1 bg-brand-primary text-white py-4 text-[10px] font-black uppercase tracking-widest shadow-lg shadow-brand-primary/10 hover:bg-[#3d5a4a] transition-colors"
                  >
                    Submit Approval
                  </button>
                  <button 
                    onClick={() => handleReject(biz.id)}
                    className="flex-1 bg-white text-gray-400 py-4 text-[10px] font-black uppercase tracking-widest border border-gray-100 hover:bg-red-50 hover:text-red-500 hover:border-red-100 transition-all"
                  >
                    Reject
                  </button>
                </div>
                  ...
                  <tr key={pickup.id} className="group hover:bg-[#F9F7F2] transition-colors">
                    ...
                    <td className="px-8 py-8">
                      <div className="flex items-center gap-3 text-gray-600">
                        ...
                        <span className="text-sm font-bold tracking-tight">{pickup.pickupDate}</span>
                      </div>
                    </td>
                    <td className="px-8 py-8">
                    ...
                    <td className="px-8 py-8 text-right">
                      <p className="text-sm font-black text-brand-primary mb-1">{pickup.amount}</p>
                      <button className="text-[10px] font-black text-gray-400 hover:text-brand-primary uppercase tracking-widest">View Details</button>
                    </td>
                  </tr>
                ))}
              </tbody>
      ...
      {activeTab === 'tracking' && (
        <div className="grid grid-cols-3 gap-8">
          {[
            { label: 'Pending Settlement', value: '4 Requests', icon: CreditCard, color: 'text-orange-500' },
            { label: 'Pickups Scheduled', value: '2 Today', icon: Clock, color: 'text-brand-primary' },
            { label: 'Total Volume (MTD)', value: 'Rs. 45,800', icon: CheckCircle2, color: 'text-green-600' },
          ].map((stat, i) => (
            <div key={i} className="bg-white p-8 border border-[#E5E1D8] flex justify-between items-center shadow-sm">
              <div>
