...
export default function MunicipalHistoryOverview() {
  ...
  const getStatusColor = (status: string) => {
    ...
    if (['pending', 'in_progress', 'unpaid', 'Accepted'].includes(status)) return 'bg-yellow-100 text-yellow-800';
    ...
    <div className="space-y-10 max-w-[1200px] mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div className="space-y-2">
          ...
          <h1 className="text-5xl font-black text-[#2D3128] tracking-tighter uppercase italic leading-none">
            History Feed<span className="text-[#4D5443]">.</span>
          </h1>
          <p className="text-sm font-medium text-[#7A7D74]">Real-time regional activities, waste logs, and enforcement history.</p>
        </div>

        <div className="flex items-center gap-3 bg-white p-1.5 rounded-none border border-[#D9D7CE]">
      ...
      <div className="flex flex-wrap items-center gap-2 pb-2 border-b-2 border-[#E5E2D9]">
        <div className="flex items-center gap-3 mr-4 text-[10px] font-black uppercase tracking-widest text-gray-400">
        ...
        {(['all', 'complaint', 'business', 'enforcement', 'logistics'] as ActivityType[]).map((t) => (
          <button
            ...
            className={`px-4 py-2 text-[10px] font-black uppercase tracking-widest transition-all ${
              ...
              : "text-[#7A7D74] bg-white border border-gray-100 hover:bg-[#EBE9E0]"
            }`}
          >
            {t}
          </button>
          ...
          <AnimatePresence mode="popLayout">
            {loading ? (
              <div className="flex flex-col items-center justify-center py-24 gap-4">
                ...
                <p className="text-xs font-black text-[#7A7D74] uppercase tracking-widest">Processing Audit Records...</p>
              </div>
            ) : filteredActivities.length === 0 ? (
              <div className="text-center py-24 bg-white border-2 border-dashed border-[#D9D7CE]">
                <p className="text-sm font-bold text-[#7A7D74] italic">No activity matching your current criteria.</p>
              </div>
                    ...
                    <div className="flex items-start gap-5">
                      ...
                      <div className="space-y-1">
                        <div className="flex flex-wrap items-center gap-2">
                           ...
                           <span className="text-[10px] font-bold text-[#7A7D74] flex items-center gap-1.5 ml-2">
